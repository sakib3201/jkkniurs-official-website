      ...
      <header className="absolute inset-x-0 top-0 z-50">
        <nav className="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
          ...
          <div className="flex lg:hidden">
            ...
            >
              <Bars3Icon className="h-6 w-6 hover:text-red-400" aria-hidden="true" />
            </button>
          </div>
          ...
          <div className="hidden lg:flex lg:gap-x-20 ">
            {pages.map((item) => (
              <Link key={item.name} href={item.href} className={`text-sm leading-6 underline-effect ${isLinkActive(item.href) ? 'active-link' : ''}`}>
                {item.name}
              </Link>
            ))}
          </div>
          ...
          <div className="hidden lg:flex lg:flex-1 lg:justify-end">
            <Link href={loginPage.href}>
              <button type="button" className="relative rounded-full text-sm font-bold px-3 py-1 text-center text-white bg-red-400 hover:bg-gray-600">
                {loginPage.name}
              </button>
            </Link>
          </div>
          ...
          <div className="fixed inset-0 z-50">
            <DialogPanel transition className="fixed pointer-events-auto max-w-md transform transition duration-500 ease-in-out data-[closed]:translate-x-full sm:duration-700  inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
              <div className="flex items-center justify-between">
                ...
                >
                  <XMarkIcon className="h-6 w-6 hover:text-red-400" aria-hidden="true" />
                </button>
              </div>
                  ...
                  <div className="space-y-2 py-6">
                    {pages.map((item) => (
                      <Link
                        key={item.name}
                        href={item.href}
                        className="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-gray-900 hover:bg-red-400 hover:text-white"
                        onClick={() => setMobileMenuOpen(false)}
                      >
                        {item.name}
                      </Link>
